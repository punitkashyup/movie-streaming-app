...
variable "mediaconvert_queue_priority" {
  ...
  validation {
    ...
    error_message = "Queue priority must be between -50 and 50"
  }
}
...
variable "enable_backend_autoscaling" {
  description = "Whether to enable auto scaling for the backend ECS service"
  type        = bool
  default     = false
}
...
variable "api_secret_key" {
  description = "Secret key for the API (leave empty to generate a random one)"
  type        = string
  default     = null
  sensitive   = true
}
